...
console.log(chalk.blue(`Compiling 'dist' js files`))
// build files used for overrides
await $`NODE_ENV=production RBC_CJS_BUILD=true babel src --out-dir dist`
console.log(chalk.blue(`Copying SASS files to 'dist'`))
// and since we don't currently use CSS modules...
await fs.copy('./src/sass', './dist/sass')
console.log(chalk.blue(`...and the 'Add-on' SASS`))
// don't forget DnD
await fs.copy(
  './src/addons/dragAndDrop/styles.scss',
  './dist/addons/dragAndDrop/styles.scss'
)
console.log(chalk.blue('Now we will build some CSS'))
// Compile SASS from './dist' to get sourcemaps
console.log(chalk.blue('Compile base styles'))
await $`sass ./dist/sass/styles.scss ./dist/css/react-big-calendar.css`
console.log(chalk.blue('Compile Add-on styles'))
// don't forget DnD
await $`sass ./dist/addons/dragAndDrop/styles.scss ./dist/addons/dragAndDrop/styles.css`
